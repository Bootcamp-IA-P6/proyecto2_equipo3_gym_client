...
//         api.get('/users/', { params: { is_active: false, skip, limit } }),
//         userClassService.getAllUserClasses(), 
//         classService.getAllClasses()
//       ]);

//       const todosLosUsuarios = [...activos, ...inactivosResponse.data];

//       // 2. Cruce de datos para obtener nombres de clases
//       const usuariosProcesados = todosLosUsuarios.map(user => {
//         // Filtramos inscripciones usando == para evitar errores de tipo (string/number)
//         const susInscripciones = todasLasInscripciones.filter(ins => ins.user_id == user.id);
        
//         const nombresClases = susInscripciones.map(ins => {
//           // Buscamos la clase en la lista maestra por ID
//           const claseEncontrada = todasLasClases.find(c => c.id == ins.class_id);
//           // Si la encuentra pone el nombre, si no, el ID como respaldo
//           return claseEncontrada ? claseEncontrada.name : `Clase #${ins.class_id}`;
//         });
...
//     setUserToEdit(null); 
//     // Damos un pequeño respiro al backend antes de recargar
...
//               <th>Email</th>
//               <th>Rol</th>
//               <th style={{ width: '180px' }}>Clases Inscritas</th>
//               <th>Estado</th>
//               <th>Acciones</th>
//             </tr>
//           </thead>
...
//                 <td>
//                   <div className="action-buttons">
//                     <button className="icon-btn edit-btn" onClick={() => handleEdit(user)}>
//                       <FiEdit size={18} />
//                     </button>
//                     <button className="icon-btn delete-btn" onClick={() => handleDelete(user)}>
//                       <FiTrash2 size={18} />
//                     </button>
//                   </div>
//                 </td>
...
//         <button onClick={prevPage} disabled={currentPage === 0} className="add-btn">
//           <FiChevronLeft /> Anterior
//         </button>
//         <span style={{ alignSelf: 'center' }}>Página {currentPage + 1}</span>
//         <button onClick={nextPage} disabled={users.length < limit} className="add-btn">
//           Siguiente <FiChevronRight />
//         </button>
//       </div>
...
const UserTable = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const [activos, inactivosResponse, todasLasInscripciones, todasLasClases] = await Promise.all([
        userService.getAllUsers(skip, limit),
        api.get('/users/', { params: { is_active: false, skip, limit } }),
        userClassService.getAllUserClasses(0, 500), 
        classService.getAllClasses()
      ]);
      ...
      const usuariosProcesados = todosLosUsuarios.map(user => {
        // Buscamos inscripciones donde el user_id coincida (forzamos string para seguridad)
        const susInscripciones = todasLasInscripciones.filter(ins => 
          String(ins.user_id) === String(user.id)
        );
        
        const nombresClases = susInscripciones.map(ins => {
          // Buscamos el nombre en el maestro de clases usando el ID que devuelva el backend
          const targetId = ins.gym_class_id || ins.class_id;
          const claseMaestra = todasLasClases.find(c => String(c.id) === String(targetId));
          
          if (claseMaestra) return claseMaestra.name;
          if (ins.gym_class?.name) return ins.gym_class.name;
          return `Clase #${targetId}`;
        });
  ...
  const handleEditSuccess = () => {
    setIsEditModalOpen(false);
    setUserToEdit(null);
    // Delay de seguridad para que el backend asiente los datos
    setTimeout(() => {
        fetchUsers();
    }, 600); 
  };
  ...
  if (loading) return <p style={{color:'white', textAlign:'center', marginTop: '20px'}}>Sincronizando...</p>;
  ...
  return (
    <div className="table-container">
      ...
      <div className="pagination-controls" style={{ display: 'flex', justifyContent: 'center', gap: '15px', marginTop: '20px' }}>
        <button onClick={() => setCurrentPage(prev => Math.max(0, prev - 1))} disabled={currentPage === 0} className="add-btn">
          Anterior
        </button>
        <span style={{ alignSelf: 'center' }}>Página {currentPage + 1}</span>
        <button onClick={() => setCurrentPage(prev => prev + 1)} disabled={users.length < limit} className="add-btn">
          Siguiente
        </button>
      </div>
